...
import {
  T_PriorityItems,
  T_ResponseGetPriorityMenuNavbar,
} from '@/api/navbar-menu/main-navbar/priority-navbar/api.get-priority-menu-navbar.type';
...
export default function PriorityHeader({
  ...
  const generateLinkBottom = (
    item: T_ResponseGetPriorityMenuNavbar[number]
  ) => {
    if (!item) {
      return '';
    }
            ...
            <div className="w-[21vh] flex items-center space-x-1">
              ...
              <Link href="/bri-prioritas">
                {priorityLogo?.field_logo_alternative?.[0]?.thumbnail?.[0]
                  ...
                  <Image
                    alt="logo-bri"
                    src={`${process.env.NEXT_PUBLIC_SELF_BASE_URL}/api/file/?path=${priorityLogo?.field_logo_alternative?.[0]?.thumbnail?.[0]?.uri?.[0]?.url ?? ''}`}
                    width={150}
                    height={60}
                ...
                {headerBottom?.map((item, index) => {
                  return (
                    <div
                      key={index}
                      className={[
                        `${pathname === item?.relative ? 'pb-2 border-b-4  border-prioritycolor mx-5' : 'pb-2 border-b-4 border-transparent hover:border-prioritycolor mx-5'}`,
                            ...
                            >
                              ...
                              {activeTab(item.relative) ? (
                                <motion.div
                                  ...
                                  className="absolute left-0 -bottom-2 h-1 rounded-full w-full bg-prioritycolor"
                                />
                              ) : (
                                <motion.span
                                  className="absolute left-0 bottom-0 w-0 h-[0.125rem] bg-prioritycolor transition-all group-hover:w-full"
                                  layoutId="underline"
