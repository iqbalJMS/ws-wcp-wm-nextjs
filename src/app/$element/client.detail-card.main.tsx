          ...
          <div className='flex-1 px-10 mdmax:px-0'>
            <div>
              <div className='mb-20 mdmax:mb-10'>
                <div className='text-privatecolor uppercase'>{card.category}</div>
                <div className='text-2xl mb-5'>{card.title}</div>
                <div className='text-lg'>{parseHTMLToReact(card.description)}</div>
              </div>
              <div>
                <Link href={card.button.link} extern={card.button.external}>
                  <ButtonSecondary rounded='full' color='privatecolor'>{card.button.text}</ButtonSecondary>
                </Link>
              </div>
            </div>
          </div>
          ...
          <div className='px-40 mdmax:px-0'>
            ...
            {tnc.details.map((detail, index) => (
              <div key={index} className='flex mdmax:flex-col'>
                <div className='text-privatecolor w-[20%] mdmax:w-full mdmax:border-transparent flex-none text-lg border-r border-black py-5'>{detail.title}</div>
                <div className=' text-lg text-black text-opacity-70 pl-5 py-5 mdmax:p-0'>{detail.description}</div>
              </div>
            ))}
            </div>
          </div>
